...
for n=1:length(range)
    ...
    for i = 1:N
        ...
        I0 = c1 .* exp(-2.*((X-p_x(i)).^2+(Y-p_y(i)).^2)./(W0.^2));
        
       
        I4 = c1 .* exp(-2.*((X-p_x(i)).^2 + (Y-p_y(i)).^2)./(W0.^2)) .* exp((-alpha.*range(n)) .* exp(((n_T.*alpha.*range(n).^2)./(2.*n_0.*rho_0.*Cp.*V))...
            .* ((((-8.*c1.*(X-p_x(i)))./(W0.^2)) .* exp(-((X-p_x(i)).^2+(Y-p_y(i)).^2)./(W0.^2))) + (((c1.*sqrt(2).*pi)./(W0.^3)).*(-W0.^2 + 8.*(Y-p_y(i)).^2) .* exp(-2.*((Y-p_y(i)).^2)./(W0.^2)) .* (1 + erf((sqrt(2).*(X-p_x(i))./W0))))))); 
        
        I_combined= I_combined + I4;
...
I_spot = @(x,y) (c1 .* exp(-2.*(x.^2 + y.^2)./(W0.^2)) .* exp(-alpha.*range(n)) .* exp((((n_T.*alpha.*range(n).^2)./(2.*n_0.*rho_0.*Cp.*V))...
            .* ((((-8.*c1.*x)./(W0.^2)) .* exp(-(x.^2+y.^2)./(W0.^2))) + (((c1.*sqrt(2).*pi)./(W0.^3)).*(-W0.^2 + 8.*y.^2) .* exp(-2.*(y.^2)./(W0.^2)) .* (1 + erf((sqrt(2).*x./W0)))))))).^2; 
      
I_spot_x = @(x,y) x.*(c1 .* exp(-2.*(x.^2 + y.^2)./(W0.^2)) .* exp(-alpha.*range(n)) .* exp((((n_T.*alpha.*range(n).^2)./(2.*n_0.*rho_0.*Cp.*V))...
            .* ((((-8.*c1.*x)./(W0.^2)) .* exp(-(x.^2+y.^2)./(W0.^2))) + (((c1.*sqrt(2).*pi)./(W0.^3)).*(-W0.^2 + 8.*y.^2) .* exp(-2.*(y.^2)./(W0.^2)) .* (1 + erf((sqrt(2).*x./W0)))))))).^2; 
I_spot_y = @(x,y) x.*(c1 .* exp(-2.*(x.^2 + y.^2)./(W0.^2)) .* exp(-alpha.*range(n)) .* exp((((n_T.*alpha.*range(n).^2)./(2.*n_0.*rho_0.*Cp.*V))...
            .* ((((-8.*c1.*x)./(W0.^2)) .* exp(-(x.^2+y.^2)./(W0.^2))) + (((c1.*sqrt(2).*pi)./(W0.^3)).*(-W0.^2 + 8.*y.^2) .* exp(-2.*(y.^2)./(W0.^2)) .* (1 + erf((sqrt(2).*x./W0)))))))).^2; 
...
I_spot_Wx = @(x,y) (x-jx).^2 .*(c1 .* exp(-2.*(x.^2 + y.^2)./(W0.^2)) .* exp(-alpha.*range(n)) .* exp((((n_T.*alpha.*range(n).^2)./(2.*n_0.*rho_0.*Cp.*V))...
            .* ((((-8.*c1.*x)./(W0.^2)) .* exp(-(x.^2+y.^2)./(W0.^2))) + (((c1.*sqrt(2).*pi)./(W0.^3)).*(-W0.^2 + 8.*y.^2) .* exp(-2.*(y.^2)./(W0.^2)) .* (1 + erf((sqrt(2).*x./W0)))))))).^2; 
I_spot_Wy = @(x,y) (y-jy).^2 .*(c1 .* exp(-2.*(x.^2 + y.^2)./(W0.^2)) .* exp(-alpha.*range(n)) .* exp((((n_T.*alpha.*range(n).^2)./(2.*n_0.*rho_0.*Cp.*V))...
            .* ((((-8.*c1.*x)./(W0.^2)) .* exp(-(x.^2+y.^2)./(W0.^2))) + (((c1.*sqrt(2).*pi)./(W0.^3)).*(-W0.^2 + 8.*y.^2) .* exp(-2.*(y.^2)./(W0.^2)) .* (1 + erf((sqrt(2).*x./W0)))))))).^2; 
...
I_free = @(x,y) (c1 .* exp(-2.*(x.^2 + y.^2)./(W0.^2))).^2;
I_free_x = @(x,y) x.*(c1 .* exp(-2.*(x.^2 + y.^2)./(W0.^2))).^2;
I_free_y = @(x,y) x.*(c1 .* exp(-2.*(x.^2 + y.^2)./(W0.^2))).^2;
...
I_free_Wx = @(x,y) (x-jx_free).^2 .*(c1 .* exp(-2.*(x.^2 + y.^2)./(W0.^2))).^2;
I_free_Wy = @(x,y) (y-jy_free).^2 .*(c1 .* exp(-2.*(x.^2 + y.^2)./(W0.^2))).^2;
...
%% Plot the output
    ...
    if(range(n) == 0 || range(n) == 10 || range(n) == 3000 || range(n) == 5000 || range(n) == 10000)
        figs(1)=figure;
        ...
        %print figures to files
         for i_fig=1:length(figs)
            print(figs(i_fig), '-dpng', sprintf('LG00_[z=%dkm].png', N, range(n)/1000, i_fig), '-r300');
        end

   end
        %     dump values to excel file
        T = table(N, P, range(n), 00 ,Ipeak_com, jx, jy, Wx, Wy, Wx_free, Wy_free, Wx/Wx_free, Wy/Wy_free, Wy/Wx);
